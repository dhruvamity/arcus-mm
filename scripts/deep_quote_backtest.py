...
def evaluate(tp, res, params, start_ns=0):
    ...
    f = np.array(res.fills, dtype=float).reshape(-1, 5)
...
def main():
    ...
    for m in a.markets.split(","):
        ...
        for d in [float(x) for x in a.depths.split(",")]:
            p = Params(depth_bps=d, requote_bps=a.requote, skew_bps=a.skew, daily_stop_usd=a.stop_usd, clip_usd=a.clip, max_pos_usd=a.max_pos, rtt_ms=a.rtt,
    ...
    L = ["# Deep-quote replay on recorded L2 tape", "",
         ...
         f"skew {a.skew:g} bps, daily stop ${a.stop_usd:g}, maker fee {a.maker_fee:g} bps, {'RTH-only for equity perps' if a.rth_only else 'all hours'}"
