...
def main():
    ...
    for m in a.markets.split(","):
        ...
        for d in [float(x) for x in a.depths.split(",")]:
            p = Params(depth_bps=d, requote_bps=a.requote, skew_bps=a.skew, clip_usd=a.clip, max_pos_usd=a.max_pos, rtt_ms=a.rtt,
    ...
    L = ["# Deep-quote replay on recorded L2 tape", "",
         ...
         f"skew {a.skew:g} bps, maker fee {a.maker_fee:g} bps, {'RTH-only for equity perps' if a.rth_only else 'all hours'}"
