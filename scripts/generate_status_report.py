...
def generate_status_markdown(output_path: Path) -> None:
    ...
    md = [
        ...
        "## 5. Key Takeaway",
        "",
        f"Audit status tracks {total_findings} findings across Mandates v3, v4, and v5. {fixed_findings} findings stand verified and remediated; {open_findings} findings are currently open for remediation under Mandate v5 Workstream A. Live execution is operating under PRE-FIX status with zero real orders placed. Tape recorder PID {rec_pid} remains continuously active.",
        "",
    ]
