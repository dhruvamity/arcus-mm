...
| **Host Latency** | Empirical RTT distribution ($p50=173.99\text{ms}, p95=386.86\text{ms}$, stress=$886.86\text{ms}$) | `latency/latency_summary.json` | `PROVISIONAL` | Measured from local development machine against `/v1/time`. Order-path ACK latency remains PROVISIONAL per APPROVE_TESTNET_FAUCET_FUNDING=NO. |
| **Regime Stratification** | Weekday US-RTH vs Weekend vs Off-hours | `src/session_calendar.py` | `YES` | Weekend results cannot transfer to weekdays |